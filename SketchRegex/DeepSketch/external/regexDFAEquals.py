...
def silent_regex_equiv(gold, predicted):
    # The semantic of & in regex_dfa_equals.jar is "concatenating", 
    # while in python re, & simply means &, the character itself. 
    # e.g. regex_dfa_equals.jar do not differentiate between ((dog){6,})&(.*[A-Za-z].*) ((dog){6,})&([A-Za-z].*..*)
    if gold == predicted:
        return "perfect"
    try:
        out = subprocess.check_output(
